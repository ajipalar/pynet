from pyext.src.typedefs import (KeyArray, 
        DeviceArray, Dimension, 
        Output, PRNGKey, State 
)
...
def generic_gibbsf(key : PRNGKey,
                   nsamples : int,
                   thin_every : int,
                   nparams : Dimension,
    ...
    def outer_body_fun(i: int, val) -> tuple[PRNGKey, PRNGKey, int, DeviceArray, DeviceArray]:

        thin_every = val[2]
    
        key, k1, thin_every, params, samples = jax.lax.fori_loop(0, thin_every, inner_body_fun, val)
        samples = samples.at[i].set(params)
    ...
    def inner_body_fun(i: int, 
                       init_val
                       ) -> tuple[PRNGKey, PRNGKey, int, DeviceArray, DeviceArray]:
