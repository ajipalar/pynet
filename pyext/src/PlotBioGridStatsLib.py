...
def _get_vec_minus_s(s : Index,  # base 1 index
                     vec : Vector, 
                     vec_l : int ) -> Vector:
...
def get_random_phi_matrix(key : PRNGKey, 
                       p : Dimension) -> Matrix:
...
def get_random_X_matrix(key : PRNGKey,
                        p : Dimension,
                        n : Dimension,
                        lam : float
                        ) -> Matrix:
...
def get_eta1(phi : Matrix,  # R^{p x p} 
...
def _get_eta2(theta_s : float,  # theta at index s-1 
