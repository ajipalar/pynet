...
def gen_excelpath_sheetname_pairs(
    dirpath: DirPath,
) -> Iterator[Tuple[FilePath, Iterator[str]]]:
...
def gen_filter_xls_columns(
    it: Iterator[List[str]], column_positions: List[int]
) -> Iterator[List[str]]:
...
def parse_lip_xls_file(
    xlspath: FilePath, cols: List[AnyCol] = desired_columns()
...
def gen_helper_protein_name_columns(
    xlspath: FilePath, cols=["PG.ProteinGroups"]
) -> Iterator[List[PGGroupCol]]:
    return parse_lip_xls_file(xlspath, cols=cols)


def gen_parse_protein_names_from_it(
    col_it: Iterator[List[PGGroupCol]],
) -> Iterator[List[str]]:
    # Yeild the header
    yield next(col_it)
...
def unique_protein_names(
    protein_name_it: Iterator[List[str]],
) -> Tuple[PGGroupCol, Set[UID]]:
...
def df_from_it(xls_it: Iterator[List[str]]):
    ...
    dtypes = {
        "R.Condition": "category",
        "R.Replicate": "category",
...
def load_lip_datasets(
    lipdirpath: DirPath, prints=False
) -> Iterator[Tuple[FilePath, Set[UID]]]:

    lip_xls_paths = excelpaths(lipdirpath.iterdir())

    def gen_protein_sets(
        x: Iterator[FilePath], prints=prints
    ) -> Iterator[Tuple[FilePath, Set[UID]]]:
        for path in x:
            if prints:
                print(f"wrapping {path}")
...
def load_stuk_dataset(stukpath: FilePath) -> Dict[Organism, Dict[Bait, Set[PreyUID]]]:
    stuk_significant_apms = pd.read_excel(
        stukpath,
        usecols=["bait_organism", "bait_name", "majority_protein_acs"],
    ...
    stuk_preys_dict = {
        org: {bait: set() for bait in unique_baits} for org in unique_organisms
    }
...
def do_comparison(
    stukpath: FilePath, gordonpath: FilePath, lippath: DirPath, prints=False
):
